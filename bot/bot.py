...
# --- Убедитесь, что эти импорты корректны для вашей структуры проекта ---
...
def find_orders_by_user(uid): 
    ...
    return {
        101: {'status': 'waiting_payment', 'reservation_expires_at': (datetime.now() + timedelta(minutes=30)).timestamp(), 'price': 10, 'product_name': 'Товар А'},
        102: {'status': 'paid', 'reservation_expires_at': (datetime.now() - timedelta(hours=2)).timestamp(), 'price': 20, 'product_name': 'Товар Б'},
        103: {'status': 'waiting_payment', 'reservation_expires_at': (datetime.now() - timedelta(hours=2)).timestamp(), 'price': 30, 'product_name': 'Товар В (ИСТЕКШИЙ)'},
    }
def get_order(order_id): return {}
def add_order(uid, data): return 104 # ID нового заказа
def get_all_stores(): return [{'store_id': 1, 'title': 'Город А'}, {'store_id': 2, 'title': 'Город Б'}]
def get_products_by_store(store_id): 
    if store_id == 1:
        return [{'product_id': 10, 'name': 'Шишка (1г)', 'price': 50}, {'product_id': 11, 'name': 'Лист (5г)', 'price': 100}]
    return []
...
# -------------------------
ADDRESSES = ["Бульвар Шевченко, Клад 1", "Ул. Победы, Тайник 2", "Проспект Мира, Локация 3"]
user_state = {} 
...
def back_to_main_menu_inline():
    ...
    return types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 Главное меню", callback_data="cmd_main_menu"))
...
def cmd_main_menu_callback(call):
    ...
    bot.send_message(call.message.chat.id, "Вы в главном меню. Выберите действие:", reply_markup=main_menu())
...
def handle_buy_button(message):
    ...
    if not stores:
        return bot.send_message(uid, "❌ Каталог магазинов пуст.", reply_markup=main_menu())
        
    joke = random.choice(grinch_jokes)
    
    markup_buttons = [
        types.InlineKeyboardButton(store["title"], callback_data=f"store_{store['store_id']}") 
        for store in stores
    ]
        
    markup = create_inline_markup_with_back(markup_buttons, back_callback_data="cmd_main_menu")
        
    bot.send_message(uid, f"{joke}\n\nВыберите магазин:", reply_markup=markup, parse_mode="Markdown")
...
def handle_back_to_buy(call):
    ...
    bot.edit_message_text(f"{joke}\n\nВыберите магазин:", call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode="Markdown")
...
def handle_store_selection(call):
    ...
    if not products:
        return bot.edit_message_text("❌ Товары в этом магазине отсутствуют.", call.message.chat.id, call.message.message_id, reply_markup=back_to_main_menu_inline())
        
    markup_buttons = [
        # В callback_data передаем store_id для кнопки "Назад"
        types.InlineKeyboardButton(product["name"], callback_data=f"product_{store_id}_{product['product_id']}") 
        for product in products
    ]
        
    # Кнопка "Назад" ведет к списку магазинов (cmd_buy_callback)
    markup = create_inline_markup_with_back(markup_buttons, back_callback_data="cmd_buy_callback") 
    
    bot.edit_message_text("Выберите товар:", call.message.chat.id, call.message.message_id, reply_markup=markup)
    bot.answer_callback_query(call.id)


@bot.callback_query_handler(func=lambda call: call.data.startswith('product_') and len(call.data.split('_')) > 2)
@anti_flood
def handle_product_selection(call):
    ...
    markup = create_inline_markup_with_back(markup_buttons, back_callback_data=f"store_{store_id}")
...
def handle_address_selection(call):
    ...
    if unpaid_count >= MAX_UNPAID_ORDERS:
        bot.answer_callback_query(call.id, f"Лимит! У вас уже {MAX_UNPAID_ORDERS} неоплаченных заказов.", show_alert=True)
        bot.send_message(
            uid,
    ...
    product_description = product_details.get("description", "Описание не предоставлено.")
    
    # 3. БРОНИРОВАНИЕ И СОЗДАНИЕ ИНВОЙСА
    reservation_expires_at = datetime.now() + timedelta(hours=INITIAL_RESERVATION_HOURS) # 1 ЧАС
    ...
    try:
        ...
        bot.send_message(uid, caption_text + "\n\n❌ **ВНИМАНИЕ:** Фотография товара не найдена.", parse_mode="Markdown")
    ...
    markup.add(types.InlineKeyboardButton("🔙 Назад (Выбрать другой адрес)", callback_data=f"product_{product_id}"))
